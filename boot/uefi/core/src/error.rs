...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Error::ReadOnlyViolation => write!(
                f,
                "A write operation was attempted on a read only file or volume."
            ),
            Error::NoWriteAccess => write!(
                f,
                "A write operation was attempted on a file that was opened without write access."
            ),
            Error::NoMedia => write!(f, "The medium does not exist."),
            Error::MediaInvalidated => write!(
                f,
                "The medium is no longer valid. Possibly due to modification."
            ),
            Error::FileOnlyOperation => {
                write!(f, "The requested operation can only be done on a file.")
            }
