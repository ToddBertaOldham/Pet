...
pub struct PageTableEntry(
    ...
    #[bit_access(
        name = "user_access_allowed",
        set = true,
        index = 2,
        borrow_self = false
    )]
    #[bit_access(
        name = "write_through_enabled",
        set = true,
        index = 3,
        borrow_self = false
    )]
...
pub mod operations {
    ...
    pub unsafe fn map(
        ...
        allocator: Option<&PagingAllocator>,
    ) -> Result<(), PagingError> {
        let directory_ptr_table =
            &mut *access_sub_table(pml_4, virtual_address.pml_4_index(), allocator)?;
        let directory_table = &mut *access_sub_table(
            directory_ptr_table,
            virtual_address.directory_ptr_index(),
            allocator,
        )?;
        let table = &mut *access_sub_table(
            directory_table,
    ...
    pub unsafe fn unmap(
        ...
        let directory_ptr_table =
            &mut *access_sub_table(pml_4, virtual_address.pml_4_index(), None)?;
        let directory_table = &mut *access_sub_table(
            directory_ptr_table,
            virtual_address.directory_ptr_index(),
            None,
        ...
        let table =
            &mut *access_sub_table(directory_table, virtual_address.directory_index(), None)?;

        table[virtual_address.table_index()] = Default::default();

        Ok(())
    }
    pub unsafe fn retrieve_physical_address(
        pml_4: &mut PageTable,
        virtual_address: VirtualAddress,
    ) -> Result<*const u8, PagingError> {
        let directory_ptr_table =
            &mut *access_sub_table(pml_4, virtual_address.pml_4_index(), None)?;
        let directory_table = &mut *access_sub_table(
            directory_ptr_table,
    ...
    unsafe fn access_sub_table(
        base_table: &mut PageTable,
        index: u16,
        page_allocator: Option<&PagingAllocator>,
